...
void solve(){
    int n;
    cin>>n;
    int cnttwo = 0,cntfive = 0; // number of times 2 and 5 occurs as prime factors along with counts
    int temp = n,curPow = 1;
    while(pow(2,curPow)<=temp){
        cnttwo += temp/(pow(2,curPow));
        curPow++;
    }
    temp = n,curPow = 1;
    while(pow(2,curPow)<=temp){
        cntfive += temp/(pow(5,curPow));
        curPow++;
    }
